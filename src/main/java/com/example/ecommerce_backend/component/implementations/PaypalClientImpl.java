...
public class PaypalClientImpl implements PaypalClientInterface {
  ...
  public PaypalClientImpl(
      WebClient.Builder webClientBuilder, PaypalClientConfig paypalClientConfig) {
  ...
  public WebClient.ResponseSpec sendGetAccessToken() {
    ...
    try {
      return webClient
          .post()
          .uri(
              new URI(paypalClientConfig.getBaseUrl() + PaypalEndpoints.ACCESS_TOKEN_URL.getPath()))
  ...
  public WebClient.ResponseSpec sendCreateOrder(
    ...
    try {
      return webClient
          .post()
          .uri(
              new URI(paypalClientConfig.getBaseUrl() + PaypalEndpoints.CREATE_ORDER_URL.getPath()))
  ...
  public WebClient.ResponseSpec sendCaptureOrder(
    ...
    try {
      return webClient
          ...
          .uri(
              new URI(
                  paypalClientConfig.getBaseUrl()
                      + PaypalEndpoints.CAPTURE_ORDER_URL
                          .getPath()
                          .replace(
                              PaypalEndpoints.ORDER_ID_VAR.getPath(),
                              userPayOrderDto.getPaypalOrderId())))
          .contentType(MediaType.APPLICATION_JSON)
          .header(
              HttpHeaders.AUTHORIZATION,
              accessTokenResponseDto.getTokenType() + " " + accessTokenResponseDto.getAccessToken())
