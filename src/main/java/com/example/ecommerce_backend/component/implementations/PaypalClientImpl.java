...
public class PaypalClientImpl implements PaypalClientInterface {
	...
	@Override
	public WebClient.ResponseSpec sendCaptureOrder(AccessTokenResponseDto accessTokenResponseDto,
			UserPayOrderDto userPayOrderDto) {
		try {
			return webClient.post()
				.uri(new URI(paypalClientConfig.getBaseUrl() + PaypalEndpoints.CAPTURE_ORDER_URL.getPath()
					.replace(PaypalEndpoints.ORDER_ID_VAR.getPath(), userPayOrderDto.getPaypalOrderId())))
				.contentType(MediaType.APPLICATION_JSON)
				.header(HttpHeaders.AUTHORIZATION,
						accessTokenResponseDto.getTokenType() + " " + accessTokenResponseDto.getAccessToken())
				.accept(MediaType.APPLICATION_JSON)
