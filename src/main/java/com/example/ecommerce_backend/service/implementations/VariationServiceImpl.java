...
public class VariationServiceImpl implements VariationServiceInterface {
	...
	public VariationEntityIndexDto createVariation(VariationEntityCreateDto variationEntityCreateDto) {
		VariationEntity variationEntity = variationEntityCreateDtoMapper
			.toEntity(variationEntityCreateDto);
	...
	public List<VariationEntity> createVariationInBulk(List<VariationEntityCreateDto> variationEntityCreateDtoList) {
		List<VariationEntity> variationEntityList = variationEntityCreateDtoMapper
			.toEntityList(variationEntityCreateDtoList);
	...
	public Page<VariationEntityIndexDto> findByCondition(Pageable pageable) {
		return variationEntityRepository.findAll(pageable)
			.map(variationEntityIndexDtoMapper::toDto);
	}

	@Override
	public VariationEntityIndexDto updateVariation(VariationEntityUpdateInfoDto variationEntityUpdateInfoDto) {
		Optional<VariationEntity> variationEntity = variationEntityRepository
			.findById(variationEntityUpdateInfoDto.getId());
		if (variationEntity.isPresent()) {
			...
			return variationEntityIndexDtoMapper
				.toDto(variationEntityRepository.save(variationEntity.get()));
		}
		else {
